		...
		for (var x = 0; x < CHUNK_LENGTH; x++)
		{
			for (var z = 0; z < CHUNK_LENGTH; z++)
			{
				for (var y = 0; y < CHUNK_LENGTH; y++)
				{
					...
					BlockData eastBlock = GetBlock(new BlockCoords(x + 1, y, z));
					BlockData westBlock = GetBlock(new BlockCoords(x - 1, y, z));
					BlockData upBlock = GetBlock(new BlockCoords(x, y + 1, z));
					BlockData downBlock = GetBlock(new BlockCoords(x, y - 1, z));
